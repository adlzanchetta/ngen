#ifndef NGEN_CSVPERFEATUREDAYLYFORCING_H
#define NGEN_CSVPERFEATUREDAYLYFORCING_H
...
// This class is a copy of CsvPerFeatureForcingProvider, but with the following changes:
// 1. it assumes the CSV files has daily data, not hourly

/**
 * @brief Forcing class providing time-series precipiation forcing data to the model.
 */
class CsvPerFeatureDailyForcingProvider : public data_access::GenericDataProvider
{
    private:

    int time_step_in_seconds = 86400;  // 86400 seconds/day (24 hours * 60 minutes * 60 seconds)
    ...
    CsvPerFeatureDailyForcingProvider(forcing_params forcing_config):start_date_time_epoch(forcing_config.simulation_start_t),
    ...
    size_t get_ts_index_for_time(const time_t &epoch_time) override {
        ...
        size_t i = 0;
        time_t seconds_in_time_step = CsvPerFeatureDailyForcingProvider::time_step_in_seconds;
        ...
        if (time >= end_date_time_epoch + CsvPerFeatureDailyForcingProvider::time_step_in_seconds) {
    ...
    double get_value(const CatchmentAggrDataSelector& selector, data_access::ReSampleMethod m) override
    {
        ...
        int time_past = current_index * CsvPerFeatureDailyForcingProvider::time_step_in_seconds;
        time_t first_time_step_start_epoch = start_date_time_epoch + time_past;
        ...
        ts_involved_s = CsvPerFeatureDailyForcingProvider::time_step_in_seconds - (init_time - first_time_step_start_epoch);
        ...
        while (time_remaining > 0) {
            ...
            // next step should not exceed the end of the time series
            if (time_remaining < CsvPerFeatureDailyForcingProvider::time_step_in_seconds) {
                ts_involved_s = time_remaining;
            } else {
                ts_involved_s = CsvPerFeatureDailyForcingProvider::time_step_in_seconds;
            }
            involved_time_step_seconds.push_back(ts_involved_s);
            involved_time_step_values.push_back(get_value_for_param_name(output_name, current_index));
            time_remaining -= ts_involved_s;
        ...
        // A parameter is processed differently depending on whether it is cummulative or rate
        double value = 0;
        double multiplier;
        for (size_t i = 0; i < involved_time_step_values.size(); ++i) {
            if (is_param_sum_over_time_step(output_name)) {
                multiplier = (double)involved_time_step_seconds[i] / (double)CsvPerFeatureDailyForcingProvider::time_step_in_seconds;
            } else {
                multiplier = (double)involved_time_step_seconds[i] / (double)selector.get_duration_secs();
            }
            value += involved_time_step_values[i] * multiplier;
        }
...
#endif // NGEN_CSVPERFEATUREDAYLYFORCING_H
